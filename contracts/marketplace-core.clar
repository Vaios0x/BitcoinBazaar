;; BitcoinBazaar Marketplace Core Contract
;; Dual-token payment system (STX + sBTC)
      ...
      (let ((total-price (get price listing))
        ...
        (if (is-eq (get payment-token listing) "sBTC")
          (begin
            ...
            (try! (stx-transfer? platform-fee tx-sender .treasury))
...
(define-private (distribute-payment (token-id uint) (total-price uint) (payment-token (string-ascii 10)))
  (let (
    (platform-fee-percent u250) ;; 2.5% = 250 basis points
    (platform-fee (/ (* total-price platform-fee-percent) u10000))
    (seller-amount (- total-price platform-fee))
  )
    ...
    (if (is-eq payment-token "sBTC")
      (try! (stx-transfer? seller-amount tx-sender tx-sender)) ;; Simplified
