;; marketplace-sbtc.clar - BitcoinBazaar Marketplace with Real sBTC Integration
;; Production version with real sBTC contract
    ...
    (let ((offer (unwrap! (map-get? offers {offer-id: offer-id}) err-not-found)))
      ...
      (let ((amount (get amount offer))
        ...
        (if (is-eq (get payment-token offer) "sBTC")
          (begin
            ;; Transfer sBTC to seller
            (try! (contract-call? sbtc-contract transfer seller-amount (get buyer offer) tx-sender))
            ;; Transfer sBTC fee to marketplace
            (try! (contract-call? sbtc-contract transfer fee-amount (get buyer offer) .marketplace))
          )
          (begin
            ;; Transfer STX to seller
