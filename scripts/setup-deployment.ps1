...
if (Test-Path "settings/Testnet.toml") {
    Write-Host "✅ Archivo Testnet.toml encontrado" -ForegroundColor Green
} else {
    Write-Host "❌ Archivo Testnet.toml no encontrado" -ForegroundColor Red
    exit 1
}
...
if ($currentMnemonic -match "abandon abandon abandon") {
    Write-Host "⚠️  Mnemonic actual es de ejemplo (no funcional):" -ForegroundColor Red
